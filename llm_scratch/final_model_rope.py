...
import math
from .helper_rope import precompute_freqs_cis, apply_rotary_emb
...
class MultiHeadAttentionKVCache(nn.Module):
    ...
    def forward(self, x, kv_cache=None, use_cache=False):
        ...
        # If we have cached K/V, we need to apply RoPE starting from past_len
        past_len = 0
        ...
        # If past_len=10 and T=1, we use freqs_cis[10:11] for position 10
        freqs_cis = self.freqs_cis[past_len:past_len + T]  # (T, head_dim//2)
...
class DecoderBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.1, max_seq_len=512):
        ...
        self.attn = MultiHeadAttentionKVCache(embed_dim, num_heads, dropout, max_seq_len)
...
class TinyGPT(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_heads, num_layers, max_seq_len=512, dropout=0.1):
        ...
        self.blocks = nn.ModuleList([
            DecoderBlock(embed_dim, num_heads, dropout, max_seq_len) for _ in range(num_layers)
        ])
