...
with torch.no_grad():
    for step in range(config['max_length']):
        ...
        if step > 0:  # First step has nothing to compare against
            ...
            flops_with_cache = calculate_flops(1, config['embed_dim'], config['num_heads'])  # Only process new token
...
def plot_comparison(cache_metrics, no_cache_metrics):
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{base_path}_plots/time_comparison.png")
    plt.close()
